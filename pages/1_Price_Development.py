...
with col2:
    ...
    default_values = [shop for shop in default_values if shop in multiselect_options][:2]
    ...
    selected_shops = st.multiselect('Select shops to compare', multiselect_options, default=default_values)
...
filtered_df.loc[:, 'date'] = pd.to_datetime(filtered_df['date'])  # Convert the date column to datetime

# Create a line plot
fig = go.Figure()
colors_p = ['#7d98a1', '#343499', '#fbe059', '#86d277', '#9fb3ba', '#7676bb', '#fbe572', '#a1dd96']  # Add more colors if needed
colors_d = ['#7d98a1', '#343499', '#fbe059', '#86d277', '#9fb3ba', '#7676bb', '#fbe572', '#a1dd96']  # Add more colors if needed
for i, shop in enumerate(selected_shops):
    shop_data = filtered_df[filtered_df['shop'] == shop]
    fig.add_trace(go.Scatter(x=shop_data['date'], y=shop_data['price'], name=shop, mode='lines', 
                             line=dict(dash='dash', color=colors_p[i])))
    fig.add_trace(go.Scatter(x=[shop_data['date'].iloc[-1]], y=[shop_data['price'].iloc[-1]], 
                             name=shop, mode='markers', showlegend=False, marker=dict(color=colors_p[i])))
    if with_delivery:
        fig.add_trace(go.Scatter(x=shop_data['date'], y=shop_data['price_delivery'], name=shop + ' with delivery', mode='lines', 
                                 line=dict(color=colors_d[i])))
        fig.add_trace(go.Scatter(x=[shop_data['date'].iloc[-1]], y=[shop_data['price_delivery'].iloc[-1]], 
                                 name=shop + ' with delivery', mode='markers', showlegend=False, marker=dict(color=colors_d[i])))
fig.update_layout(xaxis_title='<b>Date</b>', yaxis_title='<b>Price</b>', title=f'<b>Price development for {selected_product}</b>')
